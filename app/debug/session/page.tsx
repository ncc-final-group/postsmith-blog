...
function SyncStatus({ server, client }: { server?: string; client?: string }) {
  ...
  return <span className={`rounded px-2 py-1 text-xs ${isSync ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>{isSync ? '동기화됨' : '불일치'}</span>;
}
      ...
      <div className="space-y-4">
        <div>
          ...
          <div className="rounded bg-gray-50 p-4">
            <pre className="text-sm text-gray-700">{localStorageData?.sessionStorage ? JSON.stringify(localStorageData.sessionStorage, null, 2) : '데이터 없음'}</pre>
          </div>
        </div>
        <div>
          <h3 className="mb-2 font-medium text-gray-700">user-storage</h3>
          <div className="rounded bg-gray-50 p-4">
            <pre className="text-sm text-gray-700">{localStorageData?.userStorage ? JSON.stringify(localStorageData.userStorage, null, 2) : '데이터 없음'}</pre>
          </div>
        </div>
      </div>
    </div>
      ...
      <div className="mx-auto max-w-4xl">
        <div className="mb-6 flex items-center justify-between">
          ...
          <div className="space-x-2">
            <button onClick={handleRefresh} className="rounded-lg bg-blue-600 px-4 py-2 text-white hover:bg-blue-700">
              새로고침
            </button>
            <button onClick={handleClearSession} className="rounded-lg bg-red-600 px-4 py-2 text-white hover:bg-red-700">
              세션 초기화
            </button>
          </div>
        </div>
          ...
          <div className="space-y-6">
            ...
            <div className="rounded-lg bg-white p-6 shadow">
              <h2 className="mb-4 text-xl font-semibold text-gray-800">
                서버 세션 (Redis)
                <span className={`ml-2 rounded px-2 py-1 text-xs ${debugInfo.serverSession ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                  {debugInfo.serverSession ? '연결됨' : '연결 안됨'}
                </span>
              </h2>
              <div className="rounded bg-gray-50 p-4">
                <pre className="text-sm text-gray-700">{debugInfo.serverSession ? JSON.stringify(debugInfo.serverSession, null, 2) : '세션 정보 없음'}</pre>
              </div>
            </div>
            ...
            <div className="rounded-lg bg-white p-6 shadow">
              <h2 className="mb-4 text-xl font-semibold text-gray-800">
                클라이언트 세션 스토어
                <span className={`ml-2 rounded px-2 py-1 text-xs ${debugInfo.clientSessionStore ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                  {debugInfo.clientSessionStore ? '데이터 있음' : '데이터 없음'}
                </span>
              </h2>
              <div className="rounded bg-gray-50 p-4">
                <pre className="text-sm text-gray-700">{debugInfo.clientSessionStore ? JSON.stringify(debugInfo.clientSessionStore, null, 2) : '세션 데이터 없음'}</pre>
              </div>
            </div>
            ...
            <div className="rounded-lg bg-white p-6 shadow">
              <h2 className="mb-4 text-xl font-semibold text-gray-800">
                클라이언트 사용자 스토어
                <span className={`ml-2 rounded px-2 py-1 text-xs ${debugInfo.clientUserStore.isAuthenticated ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                  {debugInfo.clientUserStore.isAuthenticated ? '인증됨' : '인증 안됨'}
                </span>
              </h2>
              <div className="rounded bg-gray-50 p-4">
                <pre className="text-sm text-gray-700">{JSON.stringify(debugInfo.clientUserStore, null, 2)}</pre>
              </div>
            </div>
            ...
            <div className="rounded-lg bg-white p-6 shadow">
              <h2 className="mb-4 text-xl font-semibold text-gray-800">
                클라이언트 블로그 스토어
                <span className={`ml-2 rounded px-2 py-1 text-xs ${debugInfo.clientBlogStore.blogId ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                  {debugInfo.clientBlogStore.blogId ? '블로그 로드됨' : '블로그 미로드'}
                </span>
                {debugInfo.clientBlogStore.currentTheme && (
                  <span className="ml-2 rounded bg-blue-100 px-2 py-1 text-xs text-blue-800">테마: {debugInfo.clientBlogStore.currentTheme.themeName}</span>
                )}
              </h2>
              ...
              {debugInfo.clientBlogStore.currentTheme && (
                <div className="mb-4">
                  ...
                  <div className="rounded bg-gray-50 p-3">
                    <pre className="text-sm text-gray-700">{JSON.stringify(debugInfo.clientBlogStore.currentTheme, null, 2)}</pre>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="rounded-lg bg-white p-6 shadow">
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between rounded bg-gray-50 p-3">
                  ...
                  <SyncStatus server={debugInfo.serverSession?.userId} client={debugInfo.clientSessionStore?.userId} />
                </div>
                <div className="flex items-center justify-between rounded bg-gray-50 p-3">
                  <span className="font-medium">세션 스토어 ↔ 사용자 스토어</span>
                  <SyncStatus server={debugInfo.clientSessionStore?.userId} client={debugInfo.clientUserStore.userInfo?.id?.toString()} />
                </div>
                <div className="flex items-center justify-between rounded bg-gray-50 p-3">
                  ...
                  >
                    {debugInfo.clientBlogStore.blogId && debugInfo.clientBlogStore.blogInfo && debugInfo.clientBlogStore.currentTheme ? '완전히 로드됨' : '부분적 로드'}
                  </span>
                </div>
              </div>
            </div>
